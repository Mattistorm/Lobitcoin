...
#if defined(NDEBUG)
# error "Lobitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xed81c066eb4d43bbb18f6712abb3627f5a204dc60d2de9574a0d9a111e058603");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lobitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lobitcoin Signed Message:\n";
...
{
    // Lobitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lobitcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Lobitcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lobitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lobitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x198aaa690002c7a4d65cb2e35e05e296f3b82b937bca163b060f84c79e87c721");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Birth of LobitCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045879bbc69660a916519841ee1b2416a8660b902f55a24aa8c8f48e777c93c0f5a1d357adbc63a2609beffd65ed44f09f7f558d080e2c30bd57714c74d4212d7c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1530105556;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084528636;

        if (fTestNet)
        {
            block.nTime    = 1530105567;
            block.nNonce   = 385624559;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7f96ab41ae45fa1cab141aa71de46d4a129407598dc2e065b8da99bdef11f416"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc1, 0xb1, 0xd1 };// Lobitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LobitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LobitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LobitcoinMiner(CWallet *pwallet)
{
    printf("LobitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lobitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LobitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LobitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LobitcoinMiner, pwallet));
}
