...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Lobitcoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x66af80b2,0x0e12bdce,0x461ebdce,0x297dbdce,0x66af80ce
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Lobitcoin is probably already running."), addrBind.ToString().c_str());
        else
